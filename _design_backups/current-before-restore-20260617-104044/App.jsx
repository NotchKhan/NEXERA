...
function Hero() {
  ...
  return (
    <section
      ref={heroRef}
      id="hero"
      className="hero-command relative w-full flex flex-col justify-end overflow-hidden"
      style={{ minHeight: '100dvh' }}
    >
      <img
        src="/hero-command-bg.png"
        alt="NEXERA digital interface background"
        className="hero-command-image absolute inset-0 w-full h-full object-cover object-center z-0"
      />

      <div className="absolute inset-0 z-10 hero-command-grade" />
      <div className="absolute inset-0 z-10 hero-grid pointer-events-none" />
      <div className="absolute left-0 right-0 bottom-0 z-10 h-48 hero-bottom-fade pointer-events-none" />

      <div className="relative z-20 flex flex-col items-start pb-16 px-5 md:px-16 lg:px-24 max-w-3xl lg:max-w-2xl xl:max-w-3xl">
        <div ref={addToRefs} className="flex items-center gap-2 mb-5">
          <span className="section-label">Asset Defense Studio</span>
          <div className="w-6 h-px" style={{ background: 'var(--neon-green)', opacity: 0.5 }} />
          <span className="section-label" style={{ opacity: 0.4 }}>NEXERA / 2026</span>
        </div>

        <h1 className="font-sans font-black leading-[1.0] tracking-tight mb-2">
          <span
            ref={addToRefs}
          ...
          <span
            ref={addToRefs}
            className="block text-[2.75rem] sm:text-6xl md:text-8xl font-drama gradient-text"
          >
            digital growth.
          </span>
        </h1>
        ...
        <p
          ref={addToRefs}
          className="mt-5 text-sm md:text-base font-sans font-light max-w-sm md:max-w-xl leading-relaxed"
          style={{ color: 'rgba(239,246,242,0.55)' }}
        >
          Websites, SaaS dashboards, AI workflows, and automation systems shaped
          into one clean operating layer for teams that want speed without chaos.
        </p>

        <div ref={addToRefs} className="flex items-center gap-3 mt-7 flex-wrap">
          <a
            href="#contact"
            id="hero-cta-primary"
            className="btn-magnetic btn-primary flex items-center gap-2 font-sans font-bold text-sm px-6 py-3.5 rounded-xl no-underline"
          >
            <span className="btn-slide rounded-xl" style={{ background: 'rgba(255,255,255,0.15)' }} />
            Start a Project
          ...
          <a
            ...
            className="hero-secondary-link flex items-center gap-1.5 font-sans text-sm transition-colors no-underline group rounded-xl px-5 py-3.5"
            style={{ color: 'rgba(239,246,242,0.55)' }}
          >
            Explore Systems
        ...
        <div
          ref={addToRefs}
          className="flex flex-wrap items-start justify-between gap-y-5 gap-x-4 sm:gap-8 mt-10 pt-7 w-full"
          style={{ borderTop: '1px solid rgba(0,229,160,0.1)' }}
        >
          {[
            { val: '124', label: 'Projects shipped' },
            { val: '20+', label: 'AI tools used' },
            { val: '3x', label: 'Faster launch' },
          ].map((stat) => (
            <div key={stat.label} className="hero-stat flex flex-col min-w-[80px] flex-1 sm:flex-none">
              <span
                className="font-sans font-black text-xl md:text-2xl tracking-tight gradient-text"
              >
                {stat.val}
              </span>
              <span className="font-data text-[10px] sm:text-xs mt-0.5" style={{ color: 'rgba(239,246,242,0.35)' }}>
                {stat.label}
              </span>
            </div>
          ))}
      ...
      <div className="hero-console absolute bottom-16 right-24 z-20 hidden w-[390px] lg:block">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <p className="font-data text-[10px] uppercase tracking-[0.28em] text-white/35">Live Console</p>
            <h2 className="mt-1 font-sans text-xl font-black text-white">Launch readiness</h2>
          </div>
        ...
        <div className="space-y-3">
          {[
            ['UX Signal', '94%', 'w-[94%]'],
            ['Automation Map', '81%', 'w-[81%]'],
            ['Revenue Path', '88%', 'w-[88%]'],
          ].map(([label, value, width]) => (
            <div key={label} className="rounded-xl border border-white/10 bg-white/[0.045] p-3">
              <div className="mb-2 flex items-center justify-between font-data text-[10px] uppercase tracking-wider text-white/45">
                <span>{label}</span>
                <span className="text-emerald-200">{value}</span>
              </div>
              <div className="h-1.5 overflow-hidden rounded-full bg-white/10">
                <div className={`h-full rounded-full bg-gradient-to-r from-emerald-200 to-cyan-200 ${width}`} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
            <Cpu size={16} className="mb-3 text-cyan-200" />
            <p className="font-data text-[10px] uppercase tracking-wider text-white/35">Stack</p>
            <p className="mt-1 font-sans text-sm font-bold text-white/85">AI + Web + Ops</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
            <Layers size={16} className="mb-3 text-emerald-200" />
            <p className="font-data text-[10px] uppercase tracking-wider text-white/35">Mode</p>
            <p className="mt-1 font-sans text-sm font-bold text-white/85">Build to scale</p>
          </div>
        </div>
      </div>
    </section>
